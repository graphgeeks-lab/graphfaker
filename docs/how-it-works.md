...
Faker providers dominate the cost of node generation (about a millisecond per customer in the fraud pack). Parallel workers help in proportion to physical cores. A vectorised person sampler that draws names and addresses from locale tables with numpy is the planned fix for full-scale runs.
