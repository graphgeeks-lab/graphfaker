...
* ``graphfaker --version`` (``-V``) prints the version. ``graphfaker info`` prints it with the engine version, Python and platform, the versions of polars, numpy, pyarrow, faker and networkx, and which extras (``neo4j``, ``duckdb``, ``ladybug``, ``pyg``, ``osm``) are installed and what each enables. ``ladybug`` is an extra of its own now, next to ``neo4j`` and ``duckdb``.
