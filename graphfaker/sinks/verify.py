...
and run through a :class:`Backend`: the few things that differ between
Cypher dialects (how to list what is populated, whether the store has a
constraint catalogue, how a node comes back as a dict). Neo4j and
LadybugDB/Kùzu each provide one; a third sink gets verification for the
price of that adapter.
"""
...
def _aggregate_clauses(schema: dict[str, pl.DataType], variable: str, quote) -> list[str]:
    """``count``/``sum``/``min``/``max`` per column, all in one scan."""
    clauses = []
    for column, dtype in schema.items():
        ref = f"{variable}.{quote(column)}"
        clauses.append(f"count({ref}) AS {_alias('present', column)}")
        numeric = any(dtype == t for t in _NUMERIC)
        if numeric:
            clauses.append(f"sum({ref}) AS {_alias('sum', column)}")
        if numeric or dtype == pl.Date or isinstance(dtype, pl.Datetime):
            clauses.append(f"min({ref}) AS {_alias('min', column)}")
            clauses.append(f"max({ref}) AS {_alias('max', column)}")
    return clauses
...
def _first(v: Verification, name: str, backend: Backend, cypher: str, **params: Any) -> dict[str, Any] | None:
    ...
    try:
        rows = backend.run(cypher, **params)
...
def check_nodes(backend: Backend, tables: GraphTables, v: Verification) -> None:
    ...
    for label, frame in tables.nodes.items():
        clauses = ", ".join(
            ["count(n) AS total", f"count(DISTINCT n.{q(ID)}) AS distinct_ids", *_aggregate_clauses(frame.schema, "n", q)]
        )
        row = _first(v, f"content/{label}", backend, f"MATCH (n:{q(label)}) RETURN {clauses}")
        if row is None:
            continue
...
def check_edges(backend: Backend, tables: GraphTables, v: Verification) -> None:
    ...
    for rel, frame in tables.edges.items():
        ...
        clauses = ", ".join(["count(r) AS total", *_aggregate_clauses(attrs, "r", q)])
        row = _first(v, f"content/{rel}", backend, f"MATCH ()-[r:{q(rel)}]->() RETURN {clauses}")
        if row is None:
            continue
        ...
        if "tx_id" in attrs:
            ...
            dupes = _first(
                ...
                name,
                backend,
                f"MATCH ()-[r:{q(rel)}]->() WITH r.tx_id AS k, count(*) AS c WHERE c > 1 RETURN count(*) AS n",
            )
...
def check_truth(backend: Backend, tables: GraphTables, truth: dict[str, pl.DataFrame], v: Verification) -> None:
    ...
    if patterns is not None and patterns.height:
        total = backend.run(f"MATCH (p:{q(PATTERN_LABEL)}) RETURN count(p) AS n")[0]["n"]
        v.add(f"truth/{PATTERN_LABEL}.count", patterns.height, total)
        fraud = backend.run(f"MATCH (p:{q(PATTERN_LABEL)}) WHERE p.is_fraud RETURN count(p) AS n")[0]["n"]
    ...
    if members is not None and members.height:
        ...
        v.add(f"truth/{MEMBER_REL}.accounts", members["account_id"].n_unique(), row["accounts"], "accounts can be in several patterns")
    ...
    if transactions is not None and transactions.height:
        for rel, edges in tables.edges.items():
            ...
            actual = backend.run(f"MATCH ()-[r:{q(rel)}]->() WHERE r.pattern_id IS NOT NULL RETURN count(r) AS n")[0]["n"]
            v.add(f"truth/{rel}.labelled", expected.height, actual)
            if expected.height:
                flagged = backend.run(f"MATCH ()-[r:{q(rel)}]->() WHERE r.is_fraud RETURN count(r) AS n")[0]["n"]
    ...
    for name, frame in truth.items():
        if name in ("patterns", "accounts", "transactions") or "group" not in frame.columns:
            continue
        label = name.title()
        total = backend.run(f"MATCH (n:{q(label)}) RETURN count(n) AS n")[0]["n"]
