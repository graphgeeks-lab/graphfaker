...
A graph of your own from a file, and ``--workers`` that pay for themselves.
...
* A Docker image, ``ghcr.io/graphgeeks-lab/graphfaker``, for ``linux/amd64`` and ``linux/arm64``: the CLI with the DuckDB and Neo4j clients, ``graphfaker`` as the entrypoint, ``/data`` as the working directory to mount a volume on, an unprivileged user. The release workflow builds it on every push, runs a generate, a load and a verify against the built image, and publishes it from a ``v*`` tag. ``docs/get-started/docker.md`` has the commands, the memory note and a compose file with Neo4j.
